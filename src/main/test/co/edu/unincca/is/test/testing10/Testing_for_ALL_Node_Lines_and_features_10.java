package co.edu.unincca.is.test.testing10;
...
import co.edu.unincca.is.leaf.ComparisonLine;
import co.edu.unincca.is.leaf.IterateLine;
import co.edu.unincca.is.leaf.LineType;
...
public class Testing_for_ALL_Node_Lines_and_features_10 {
	public static void main(String[] args) throws IOException {
		...
		ilr.setStreamSource(Testing_for_ALL_Node_Lines_and_features_10.class
				.getResourceAsStream("Testing for ALL Node Lines and features.txt"));
		RuleSetParser isf = new RuleSetParser();
		RuleSetScanner rsc = new RuleSetScanner(ilr, isf);
		rsc.scanRuleSet();
		rsc.establishNodeSet(null);

		BufferedReader br = new BufferedReader(new InputStreamReader(Testing_for_ALL_Node_Lines_and_features_10.class
				.getResourceAsStream("Comparison Testing for ALL Node Lines and features.txt")));
		String line;
		...
		while ((line = br.readLine()) != null) {
			...
			if (!line.isEmpty()) {
				NodeObject_For_Inference_Test nfit = new NodeObject_For_Inference_Test(tempArray[0].trim(),
						tempArray[1].trim().split(":"));
		...
		IntStream.range(0, nodeListMock.size()).forEach(i -> {
			...
			if (actualNode.getNodeName().equals("person's nationality IS \"Australian\"")
				...
				comparisonTempList.add(i);
					...
					&& mockNode.equals(actualNode.getNodeName()) && actualNode.getVariableName().equals("person's name")
					&& actualNode.getFactValue().getType().equals(FactValueType.DEFI_STRING)
...

			{
				...
				comparisonTempList.add(i);
					...
					&& ((ComparisonLine) actualNode).getRHS().getValue().toString().equals("Australian")) {
				comparisonTempList.add(i);
			} else if (actualNode.getNodeName().equals("person age >18") && mockNode.equals(actualNode.getNodeName())
				...
				comparisonTempList.add(i);
					...
					&& mockNode.equals(actualNode.getNodeName())
					&& ((ComparisonLine) actualNode).getLHS()
							.equals("a number of countries the person has travelled so far")
					&& ((ComparisonLine) actualNode).getRHS().getType().equals(FactValueType.INTEGER)
					&& ((ComparisonLine) actualNode).getRHS().getValue().toString().equals("40")) {
				comparisonTempList.add(i);
			} else if (actualNode.getNodeName()
					.equals("current location of person's passport = the place the person normally locate the passport")
					&& mockNode.equals(actualNode.getNodeName())
					...
					&& ((ComparisonLine) actualNode).getRHS().getValue().toString()
							.equals("the place the person normally locate the passport")) {
				comparisonTempList.add(i);
				...
				comparisonTempList.add(i);
					...
					&& mockNode.equals(actualNode.getNodeName())
					&& ((ComparisonLine) actualNode).getLHS()
							.equals("person's legally yearly period of stay in Australia")
					&& ((ComparisonLine) actualNode).getRHS().getType().equals(FactValueType.INTEGER)
				...
				comparisonTempList.add(i);
			} else if (actualNode.getNodeName().equals(
					...
					&& actualNode.getFactValue().getType().equals(FactValueType.STRING)
					&& actualNode.getFactValue().getValue().toString().equals(
							"( number of drinks the person consumes an hour * hours of drinks a day * (5-1))")) {
				comparisonTempList.add(i);
			} else if (actualNode.getNodeName().equals("ALL service ITERATE: LIST OF service history")
					&& mockNode.equals(actualNode.getNodeName()) && actualNode.getLineType().equals(LineType.ITERATE)
				...
				comparisonTempList.add(i);
					...
					&& mockNode.equals(actualNode.getNodeName()) && actualNode.getLineType().equals(LineType.COMPARISON)
		...
		while (ie.getAssessmentState().getWorkingMemory()
				.get(isf.getNodeSet().getNodeSortedList().get(0).getNodeName()) == null
				|| !ie.getAssessmentState().allMandatoryNodeDetermined()) {
...
//				}
		...
		ie.generateSortedSummaryList().stream().forEachOrdered(node -> {
			if (workingMemory.get(node).getType().equals(FactValueType.LIST)) {
				((FactListValue<?>) workingMemory.get(node)).getValue().stream()
						.forEach(item -> System.out.println("List Value - " + node + " : " + item));
			}
			{
