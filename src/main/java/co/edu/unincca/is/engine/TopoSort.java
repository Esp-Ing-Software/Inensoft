package co.edu.unincca.is.engine;
...
import co.edu.unincca.is.leaf.DependencyMatrix;
import co.edu.unincca.is.leaf.DependencyType;
import co.edu.unincca.is.leaf.Node;
import co.edu.unincca.is.leaf.Record;
    	        ...
    	        if(!orOutDependency.isEmpty() || !andOutDependency.isEmpty())
    	        {
    	            ...
    	            if(!orOutDependency.isEmpty() && andOutDependency.isEmpty())
    	            {
    	                while(!childRuleList.isEmpty())
    	                {
    		                	 ...
    		                	 * if it is 'OR' rule and it is 'TRUE' then it is the quickest path, and ignore other 'OR' rules
    		                	 * Therefore, looking for more likely 'TRUE' rule would be the quickest one rather than
