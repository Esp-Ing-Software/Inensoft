package co.edu.unincca.is.leaf;
...
import co.edu.unincca.is.fact.FactStringValue;
import co.edu.unincca.is.fact.FactValue;
import co.edu.unincca.is.rule.Tokens;
	...
	public FactValue selfEvaluate(HashMap<String, FactValue> workingMemory, ScriptEngine nashorn)
	{
		...
		if(!this.isPlainStatementFormat)
		{
			...
			else if(tokens.tokensList.stream().anyMatch((s) -> s.equals("IS IN LIST:")))
			{
				...
				{
					FactValue variableValueFromWorkingMemory = workingMemory.get(this.variableName);
					lineValue = variableValueFromWorkingMemory != null?
					((FactListValue<?>)workingMemory.get(listName)).getValue().stream().anyMatch((factValue)->((FactStringValue)factValue).getValue().equals(variableValueFromWorkingMemory.getValue().toString()))
					:
